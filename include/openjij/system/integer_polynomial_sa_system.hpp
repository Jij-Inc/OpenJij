...
public:
    IntegerSASystem(const graph::IntegerPolynomialModel &model, const RandType::result_type seed)
            ...
            for (const auto& [index, degree] : key_value_list[i].first) {
                ...
                if (x == 0) {
                    if (this->zero_count_[i] == 1) {
                        this->base_energy_difference_[index][degree] += value * this->term_prod_[i];
                    }
                } else {
                    if (this->zero_count_[i] == 0) {
                        this->base_energy_difference_[index][degree] += value * this->term_prod_[i] / std::pow(x, degree);
                    }
                }
            }
        }
    ...
    std::int64_t GenerateCandidateValue(std::int64_t index) {
        return this->state_[index].GenerateCandidateValue(this->random_number_engine);
    }
    ...
    double GetEnergyDifference(std::int64_t index, std::int64_t new_value) const {
        ...
        for (const auto& [degree, value] : this->base_energy_difference_[index]) {
            dE += value * (std::pow(new_value, degree) - std::pow(current_value, degree));
        }
        return dE;
    }
    ...
    void SetValue(std::int64_t index, std::int64_t new_value) {
        ...
        if (current_value != 0 && new_value != 0) {
            for (const auto& [cons_ind, degree] : interactions) {
                const double a = std::pow(static_cast<double>(new_value) / current_value, degree);
                const double ddE = (a - 1) * key_value_list[cons_ind].second * this->term_prod_[cons_ind];
                ...
                for (const auto& [i, d] : key_value_list[cons_ind].first) {
                    if (i != index) {
                        if (this->state_[i].value == 0 && total_count == 1) {
                            ...
                            this->base_energy_difference_[i][d] += ddE / std::pow(this->state_[i].value, d);
                        }
                    }
                }
            }
            ...
            for (const auto& [cons_ind, degree] : interactions) {
                ...
                const double ddE = b * key_value_list[cons_ind].second * this->term_prod_[cons_ind];
            ...
            for (const auto& [cons_ind, degree] : interactions) {
                const int total_count = this->zero_count_[cons_ind];
                this->zero_count_[cons_ind]++;
                const double ddE = -key_value_list[cons_ind].second * this->term_prod_[cons_ind];
    ...
    std::pair<int, double> GetMinEnergyDifference(std::int64_t index) {
        if (this->UnderQuadraticCoeff(index)) {
            ...
            const double a = (it_a != this->base_energy_difference_[index].end()) ? it_a->second : 0.0;
            
            auto it_b = this->base_energy_difference_[index].find(1);
            const double b_base = (it_b != this->base_energy_difference_[index].end()) ? it_b->second : 0.0;
            const double b = b_base + 2 * x.value * a;

            if (a > 0) {
                const double center = -b / (2 * a);
                if (dxu <= center) {
                    return {x.upper_bound, this->GetEnergyDifference(index, x.upper_bound)};
                    ...
                    if (center - dx_left <= dx_right - center) {
                        return {x.value + dx_left, this->GetEnergyDifference(index, x.value + dx_left)};
                    } else {
                        return {x.value + dx_right, this->GetEnergyDifference(index, x.value + dx_right)};
                    }
                } else if (dxl >= center) {
                    return {x.lower_bound, this->GetEnergyDifference(index, x.lower_bound)};
                } else {
            ...
            for (std::int64_t val = this->state_[index].lower_bound; val <= this->state_[index].upper_bound; ++val) {
    ...
    double GetLinearCoeff(std::int64_t index) const {
        ...
        double c1 = (it1 != this->base_energy_difference_[index].end()) ? it1->second : 0.0;
        
        auto it2 = this->base_energy_difference_[index].find(2);
        double c2 = (it2 != this->base_energy_difference_[index].end()) ? it2->second : 0.0;
