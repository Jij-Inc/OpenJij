...
public:
   ...
   IntegerQuadraticModel(std::vector<std::vector<std::int64_t>> &key_list,
      ...
      for (std::size_t i = 0; i < key_list.size(); ++i) {
         ...
         if (key.size() == 0) {
            ...
            if (key[0] < 0 || key[0] >= this->num_variables_ || key[1] < 0 || key[1] >= this->num_variables_) {
               throw std::runtime_error("Index out of bounds.");
            }
            if (key[0] == key[1]) {
   ...
   const std::unordered_set<std::int64_t> &GetOnlyBilinearIndexSet() const { return only_bilinear_index_set_; }
