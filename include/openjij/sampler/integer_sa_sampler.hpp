...
IntegerSAResult BaseSA(const ModelType &model,
   ...
   auto get_T = [&](const std::int64_t sweep) {
      if (schedule == utility::TemperatureSchedule::LINEAR) {
         return max_T + (min_T - max_T) * (static_cast<double>(sweep) / (num_sweeps - 1));
      } else if (schedule == utility::TemperatureSchedule::GEOMETRIC) {
         return max_T * std::pow(min_T / max_T, static_cast<double>(sweep) / (num_sweeps - 1));
      }
      else {
   ...
   auto dispatch_engine = [&]<typename UpdaterType>() -> IntegerSAResult {
      switch (rand_type) {
         case algorithm::RandomNumberEngine::XORSHIFT:
            return BaseSA<ModelType, utility::Xorshift, UpdaterType>(model, schedule, num_sweeps, static_cast<utility::Xorshift::result_type>(seed), min_T, max_T, log_history);
         case algorithm::RandomNumberEngine::MT:
            return BaseSA<ModelType, std::mt19937, UpdaterType>(model, schedule, num_sweeps, static_cast<std::mt19937::result_type>(seed), min_T, max_T, log_history);
         case algorithm::RandomNumberEngine::MT_64:
            return BaseSA<ModelType, std::mt19937_64, UpdaterType>(model, schedule, num_sweeps, seed, min_T, max_T, log_history);
         default:
...
template<class ModelType>
std::vector<IntegerSAResult> SampleByIntegerSA(const ModelType &model,
                                               const std::int64_t num_sweeps,
                                               const algorithm::UpdateMethod update_method,
                                               const algorithm::RandomNumberEngine rand_type,
                                               const utility::TemperatureSchedule schedule,
...
#pragma omp parallel for schedule(guided) num_threads(num_threads)
   for (std::int64_t i = 0; i < num_reads; ++i) {
      results[i] = SolveByIntegerSA(model, num_sweeps, update_method, rand_type, schedule, seed + i, min_T, max_T, log_history);
   }
